...
jQuery(function ($) {
    ...
    function initialiseDocumentButtons() {
        if ($('.grid-download-trustpayments-invoice-row-link').length) {
            ...
            $('.grid-download-trustpayments-invoice-row-link').click(function(e) {
                e.preventDefault();
                let id_order = getOrderIdFromUrl($(this).attr('href'));
                window.open(trustpayments_admin_token + "&action=trustPaymentsInvoice&id_order=" + id_order, "_blank");
            });
        
            $('.grid-download-trustpayments-invoice-row-link').each(function() {
                let $this = $(this);
                ...
                if (!paymentStatusText.includes("Payment accepted") || isWPayment.includes("0")) {
                    $row.find('.grid-download-trustpayments-invoice-row-link').hide();
