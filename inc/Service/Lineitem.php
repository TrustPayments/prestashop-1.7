 ...
 */
class TrustPaymentsServiceLineitem extends TrustPaymentsServiceAbstract
{
    ...
    {
        $currencyCode = TrustPaymentsHelper::convertCurrencyIdToCode($cart->id_currency);
        $items = array();
        ...
        foreach ($summary['products'] as $productItem) {
            $item = new \TrustPayments\Sdk\Model\LineItemCreate();
            $totalAmount = $this->roundAmount((float) $productItem['total_wt'], $currencyCode);
            $item->setAmountIncludingTax($totalAmount);
            $name = empty($productItem['name']) ? TrustPaymentsHelper::getModuleInstance()->l(
                'Product',
                'lineitem'
            ...
            $item->setTaxes($taxes);
            $item->setType(\TrustPayments\Sdk\Model\LineItemType::PRODUCT);
            if ($productItem['id_product'] == Configuration::get(TrustPaymentsBasemodule::CK_FEE_ITEM) ||
                $productItem['id_product'] == Configuration::get(TrustPaymentsBasemodule::CK_SURCHARGE_ITEM)) {
                $item->setType(\TrustPayments\Sdk\Model\LineItemType::FEE);
        ...
        if ($shippingCosts > 0) {
            ...
            $item->setQuantity(1);
            $item->setShippingRequired(false);
            $item->setSku(TrustPaymentsHelper::getModuleInstance()->l('Shipping', 'lineitem'));
            $name = "";
            ...
            $name = empty($name) ? TrustPaymentsHelper::getModuleInstance()->l('Shipping', 'lineitem') : $name;
            $item->setName($name);
            if (! $taxCalculatorFound) {
                $taxRate = 0;
                $taxName = TrustPaymentsHelper::getModuleInstance()->l('Tax', 'lineitem');
            ...
            $item->setType(\TrustPayments\Sdk\Model\LineItemType::SHIPPING);
        ...
        if ($wrappingCosts > 0) {
            ...
            $item->setName(TrustPaymentsHelper::getModuleInstance()->l('Wrapping Fee', 'lineitem'));
            $item->setQuantity(1);
            $item->setShippingRequired(false);
        ...
        if (count($summary['discounts']) > 0) {
            ...
            foreach ($summary['discounts'] as $discount) {
                $nameBase = empty($discount['description']) ? TrustPaymentsHelper::getModuleInstance()->l(
                    'Discount',
                    'lineitem'
        ...
        if ($discountOnly && $shippingItem != null) {
            $itemFreeShipping = new \TrustPayments\Sdk\Model\LineItemCreate();
            $name = empty($discountOnly['name']) ? TrustPaymentsHelper::getModuleInstance()->l(
                'Shipping Discount',
                'lineitem'
            ) : $discountOnly['name'];
    ...
    private function extractTaxName($psTax, $cart) {
    	...
    	if (empty($translated) || strlen($translated) < 2) {
    		...
    		return TrustPaymentsHelper::getModuleInstance()->l('Tax', 'lineitem');
    	}
    	else {
    		return $translated;
    	}
    ...
    public function getItemsFromOrders(array $orders)
    {
        ...
        $cleaned = TrustPaymentsHelper::cleanupLineItems(
            $items,
            $orderTotal,
            TrustPaymentsHelper::convertCurrencyIdToCode($order->id_currency)
        );
        return $cleaned;
    }
    ...
    protected function getItemsFromOrdersInner(array $orders, $orderTotal)
    {
        ...
        foreach ($orders as $order) {
            ...
            foreach ($order->getProducts() as $orderItem) {
                ...
                $name = empty($orderItem['product_name']) ? TrustPaymentsHelper::getModuleInstance()->l(
                    'Product',
                    'lineitem'
                ...
                if ($orderItem['product_id'] == Configuration::get(TrustPaymentsBasemodule::CK_FEE_ITEM) ||
                    $orderItem['product_id'] == Configuration::get(TrustPaymentsBasemodule::CK_SURCHARGE_ITEM)) {
                    $item->setType(\TrustPayments\Sdk\Model\LineItemType::FEE);
                    $item->setShippingRequired(false);
                }
            ...
            foreach ($order->getCartRules() as $orderCartRule) {
                ...
                $nameBase = empty($orderCartRule['name']) ? TrustPaymentsHelper::getModuleInstance()->l(
                    'Discount',
                    'lineitem'
                ) : $orderCartRule['name'];
                $discountItems = $this->getDiscountItems(
        ...
        if ($cartRule->gift_product != 0) {
            foreach ($existingLineItems as $exisitingLineItem) {
                if ($exisitingLineItem->getUniqueId() ==
                    ...
                    $item->setType(\TrustPayments\Sdk\Model\LineItemType::DISCOUNT);
