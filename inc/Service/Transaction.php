 ...
 */
class TrustPaymentsServiceTransaction extends TrustPaymentsServiceAbstract
{
    ...
    protected function getTransactionService()
    {
        if ($this->transactionService === null) {
            $this->transactionService = new \TrustPayments\Sdk\Service\TransactionService(
    ...
    protected function getTransactionIframeService()
    {
        if ($this->transactionIframeService === null) {
            $this->transactionIframeService = new \TrustPayments\Sdk\Service\TransactionIframeService(
                TrustPaymentsHelper::getApiClient()
            );
        }
    ...
    protected function getTransactionPaymentPageService()
    {
        if ($this->transactionPaymentPageService === null) {
            $this->transactionPaymentPageService = new \TrustPayments\Sdk\Service\TransactionPaymentPageService(
                TrustPaymentsHelper::getApiClient()
            );
        }
    ...
    protected function getChargeAttemptService()
    {
        if ($this->chargeAttemptService === null) {
            $this->chargeAttemptService = new \TrustPayments\Sdk\Service\ChargeAttemptService(
                TrustPaymentsHelper::getApiClient()
            );
        }
    ...
    public function getJavascriptUrl(Cart $cart)
    {
        ...
        return $js . "&className=trustpaymentsIFrameCheckoutHandler";
    }
     ...
     * @param Order $order
     * @return TrustPaymentsModelTransactioninfo
     */
    public function updateTransactionInfo(\TrustPayments\Sdk\Model\Transaction $transaction, Order $order)
    {
        $info = TrustPaymentsModelTransactioninfo::loadByTransaction(
            $transaction->getLinkedSpaceId(),
            $transaction->getId()
        );
     ...
     * @return string
     */
    protected function getPaymentMethodImage(\TrustPayments\Sdk\Model\Transaction $transaction, Order $order)
    {
        if ($transaction->getPaymentConnectorConfiguration() == null) {
            $moduleName = $order->module;
            if ($moduleName == "trustpayments") {
                $id = TrustPaymentsHelper::getOrderMeta($order, 'trustPaymentsMethodId');
                $methodConfiguration = new TrustPaymentsModelMethodconfiguration($id);
                return TrustPaymentsHelper::getResourceUrl(
    ...
    public function getPossiblePaymentMethods(Cart $cart)
    {
        ...
        if (! isset(self::$possiblePaymentMethodCache[$currentCartId]) ||
            ...
            $methodConfigurationService = TrustPaymentsServiceMethodconfiguration::instance();
    ...
    {
        $ids = TrustPaymentsHelper::getCartMeta($cart, 'mappingIds');
    ...
    public function confirmTransaction(Order $dataSource, array $orders, $methodConfigurationId)
    {
        ...
        for ($i = 0; $i < 5; $i ++) {
            try {
                $ids = TrustPaymentsHelper::getOrderMeta($dataSource, 'mappingIds');
                $spaceId = $ids['spaceId'];
    ...
    protected function assembleOrderTransactionData(
        ...
        $transaction->setCurrency(TrustPaymentsHelper::convertCurrencyIdToCode($dataSource->id_currency));
        ...
        $transaction->setLanguage(TrustPaymentsHelper::convertLanguageIdToIETF($dataSource->id_lang));
        $transaction->setShippingMethod(
            $this->fixLength($this->getShippingMethodNameForCarrierId($dataSource->id_carrier), 200)
        );

        $transaction->setLineItems(TrustPaymentsServiceLineitem::instance()->getItemsFromOrders($orders));
        ...
        $transaction->setSuccessUrl(
            Context::getContext()->link->getModuleLink(
                ...
                array(
                    ...
                    'secret' => TrustPaymentsHelper::computeOrderSecret($dataSource),
                    'action' => 'success',
                    'utm_nooverride' => '1'
                ),
                true
    ...
    protected function createTransactionFromCart(Cart $cart)
    {
        ...
        $createTransaction->setDeviceSessionIdentifier(Context::getContext()->cookie->tru_device_id);

        $spaceViewId = Configuration::get(TrustPaymentsBasemodule::CK_SPACE_VIEW_ID, null, null, $cart->id_shop);
        ...
        TrustPaymentsHelper::updateCartMeta(
            $cart,
            'mappingIds',
            array(
    ...
    protected function assembleCartTransactionData(
        ...
        $transaction->setCurrency(TrustPaymentsHelper::convertCurrencyIdToCode($cart->id_currency));
        ...
        $transaction->setLanguage(TrustPaymentsHelper::convertLanguageIdToIETF($cart->id_lang));
        $transaction->setShippingMethod(
            $this->fixLength($this->getShippingMethodNameForCarrierId($cart->id_carrier), 200)
        );

        $transaction->setLineItems(TrustPaymentsServiceLineitem::instance()->getItemsFromCart($cart));
    ...
    protected function getGenderForCustomerId($id)
    {
        ...
        if ($gender->type == '0') {
            return \TrustPayments\Sdk\Model\Gender::MALE;
        } elseif ($gender->type == '1') {
            return \TrustPayments\Sdk\Model\Gender::FEMALE;
        }
        return null;
    }
