...
class TrustPaymentsOrderstatus
{
    private static $orderStatesConfig = array(
        'TRU_REDIRECTED' => array(
            'color' => '#4169e1',
            'name' => 'Trust Payments Processing',
            'invoice' => 0,
            'logable' => 0,
            'image' => 'redirected'
        ),
        'TRU_AUTHORIZED' => array(
            'color' => '#0000cd',
            'name' => 'Trust Payments Authorized',
            'invoice' => 0,
            'logable' => 1,
            'image' => 'authorized'
        ),
        ...
        'TRU_MANUAL' => array(
            'color' => '#191970',
            'name' => 'Trust Payments Manual Decision',
            'invoice' => 0,
            'logable' => 1,
            'image' => 'manual'
        )
    ...
    {
        return self::getOrderStatus('TRU_REDIRECTED');
    }

    public static function getAuthorizedOrderStatus()
    {
        return self::getOrderStatus('TRU_AUTHORIZED');
    }

    public static function getWaitingOrderStatus()
    {
        return self::getOrderStatus('TRU_WAITING');
    }

    public static function getManualOrderStatus()
    {
        return self::getOrderStatus('TRU_MANUAL');
    }
    ...
    private static function createOrderState($key)
    {
        ...
        $state->template = '';
        $state->unremovable = 1;
        $state->module_name = 'trustpayments';
        $state->add();
