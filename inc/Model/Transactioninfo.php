...
class TrustPaymentsModelTransactioninfo extends ObjectModel
{
    ...
    {
        $decoded = TrustPaymentsTools::base64Decode($this->labels, true);
        if ($decoded === false) {
            $decoded = $this->labels;
        }
    ...
    {
        $this->labels = TrustPaymentsTools::base64Encode(serialize($labels));
    }
    ...
    {
        $transactionInfos = new PrestaShopCollection('TrustPaymentsModelTransactioninfo');
        $transactionInfos->where('order_id', '=', $orderId);
        $result = $transactionInfos->getFirst();
        if ($result === false) {
            $result = new TrustPaymentsModelTransactioninfo();
        }
        return $result;
    }
